...
VARSAYILAN = {
    ...
    "once_sonra"      : True,
    # ── Telegram ──────────────────────────────────
    # BotFather'dan aldığın token ve chat ID'ini buraya yaz
    # Boş bırakırsan Telegram bildirimi devre dışı
    "telegram_token"  : "",   # örn: "7123456789:AAFxxxxxxx"
    "telegram_chat_id": "",   # örn: "123456789"
}

# ══════════════════════════════════════════════════════
#  MANTIK KATMANI  (GUI'den bağımsız)
# ══════════════════════════════════════════════════════

def telegram_gonder(token, chat_id, mesaj, resim_yolu=None):
    """Bot koruması alındığında Telegram'a mesaj + resim gönderir."""
...
def koyleri_parse_et(html, max_mesafe, max_duvar, birlik_filtre):
    ...
    for satir in tablo.find_all("tr")[1:]:
        ...
        # Birlik filtresi — VE mantığı, ayrı minimum sayılarla
        # Format: "spear:50,sword:30"  → spear<50 VEYA sword<30 ise atla
...
class TwGUI:
    ...
    def _etiket(self, parent, metin, renk=None, bold=False, size=10):
        ...
        return tk.Label(parent, text=metin, bg=self.PANEL,
                        fg=renk or self.METIN, font=font)
    ...
    def _ayirici(self, parent):
        tk.Frame(parent, bg=self.ACCENT, height=1).pack(fill="x", pady=6)

    def _panel(self, parent, baslik):
        f = tk.LabelFrame(parent, text=f"  {baslik}  ",
    ...
    def _arayuz_kur(self):
        SOL = tk.Frame(self.root, bg=self.BG, padx=10, pady=10)
        SOL.grid(row=0, column=0, sticky="nsew")
        SAG = tk.Frame(self.root, bg=self.BG, padx=10, pady=10)
    ...
    def _filtre_panel(self, parent):
        ...
        self._satir(f, "Birlik min (birlik:sayi,birlik:sayi)", self.e_birlik, 2)
    ...
    def _renk_panel(self, parent):
        ...
        for lbl, w in [("R", self.e_r), ("G", self.e_g), ("B", self.e_b), ("±", self.e_tol)]:
            tk.Label(row, text=lbl, bg=self.PANEL, fg=self.GRI,
    ...
    def _telegram_panel(self, parent):
        ...
        tk.Label(f, text="Bot koruması gelince ekran görüntüsüyle bildirim gönderir",
                 bg=self.PANEL, fg=self.GRI,
                 font=("Segoe UI", 8), wraplength=240).grid(
            row=2, column=0, columnspan=2, padx=8, pady=(0,6), sticky="w")

    def _istatistik_panel(self, parent):
        f = self._panel(parent, "📊  İstatistikler")
        f.pack(fill="x", pady=(0, 6))
        iç = tk.Frame(f, bg=self.PANEL)
        iç.pack(fill="x", padx=8, pady=6)

        def stat_kutu(parent, baslik, renk):
            k = tk.Frame(parent, bg=self.BG, padx=10, pady=6, relief="flat")
            k.pack(side="left", expand=True, fill="x", padx=4)
            tk.Label(k, text=baslik, bg=self.BG, fg=self.GRI,
                     font=("Segoe UI", 8)).pack()
            lbl = tk.Label(k, text="0", bg=self.BG, fg=renk,
                           font=("Segoe UI", 16, "bold"))
            lbl.pack()
            return lbl

        self.lbl_tur    = stat_kutu(iç, "TUR",       self.SARI)
        self.lbl_tikla  = stat_kutu(iç, "TIKLAMA",   self.YESIL)
        self.lbl_atla   = stat_kutu(iç, "ATLANAN",   self.ACCENT)

        # Durum çubuğu
        self.lbl_durum = tk.Label(f, text="⬤  Bekleniyor",
                                  bg=self.PANEL, fg=self.GRI,
                                  font=("Segoe UI", 9, "bold"))
        self.lbl_durum.pack(pady=(0, 4))

        # Geri sayım
        self.lbl_geri = tk.Label(f, text="",
                                 bg=self.PANEL, fg=self.SARI,
                                 font=("Segoe UI", 9))
    ...
    def _log_panel(self, parent):
        ...
        self.log_kutu = scrolledtext.ScrolledText(
            f, width=48, height=18,
            bg="#0d0d1a", fg=self.METIN,
            font=("Consolas", 8),
            relief="flat", bd=0,
            state="disabled"
        )
    ...
    def _kontrol_panel(self, parent):
        ...
        f.pack(fill="x", pady=6)
        self.btn_basla = tk.Button(
            f, text="▶  BAŞLAT", command=self._basla_durdur,
            bg=self.BUTON_OF, fg="white", activebackground="#1e8449",
            font=("Segoe UI", 11, "bold"), relief="flat",
            cursor="hand2", height=2, width=20
        )
        self.btn_basla.pack(side="left", padx=4)
        tk.Button(
            f, text="🗑  Logu Temizle", command=self._log_temizle,
    ...
    def _durum(self, metin, renk=None):
        self.root.after(0, lambda: self.lbl_durum.config(
            text=metin, fg=renk or self.GRI))
    ...
    def _renk_onizle(self):
        try:
            ...
            g = int(self.e_g.get())
            b = int(self.e_b.get())
            hex_renk = f"#{r:02x}{g:02x}{b:02x}"
            self.renk_onizleme.config(bg=hex_renk)
        except:
            pass
    ...
    def _dongu(self):
        ...
        if not cerez:
            self.log("❌ Çerez alınamadı! Chrome'da TribalWars açık ve giriş yapılmış olmalı.", "kirmizi")
            self._sonlandir()
            return
        ...
        while not self.dur_event.is_set():
            ...
            if tur > 1:
                pyautogui.hotkey("f5")
                self.log("F5 — sayfa yenilendi.", "gri")
                time.sleep(3)

            # Ekran görüntüsü — önce
            self._ekran_goruntus_al(tur, "once", ayar)
            ...
            if not html:
                ...
                if 'id="botprotection_quest"' in html or "botprotection_quest" in html:
                    self.log("🚨 BOT KORUMASI ALGILANDI! Script durduruluyor.", "kirmizi")
                    self._durum("🚨  BOT KORUMASI — DURDURULDU", self.ACCENT)
                    self.dur_event.set()
                    ekran_dosya = self._ekran_goruntus_al(tur, "bot_koruma", ayar)
                    # Telegram bildirimi
                    token   = ayar.get("telegram_token", "")
                    chat_id = ayar.get("telegram_chat_id", "")
                    mesaj = (
                        f"🚨 <b>TribalWars Bot Koruması!</b>\n"
                        f"⏰ Saat: {datetime.now().strftime('%H:%M:%S')}\n"
                        f"🔄 Tur: {tur}\n"
                        f"🌐 Sunucu: {ayar.get('sunucu_url','')}\n\n"
                        f"Script durduruldu. Captcha'yı çöz ve tekrar başlat."
                    )
                    threading.Thread(
                        target=telegram_gonder,
                        args=(token, chat_id, mesaj, ekran_dosya),
                        daemon=True
                    ).start()
                    self.log("📨 Telegram bildirimi gönderildi.", "sari")
                    # Sesli uyarı (Windows)
                    try:
                        import winsound
                        for _ in range(5):
                ...
                koyler = koyleri_parse_et(
                    html, ayar["max_mesafe"], ayar["max_duvar"], ayar["birlik_filtre"])
                ...
                for k in atla:
                    self.log(f"   ⏭ Atlandı ({k['atla_neden']})", "gri")

                if saldir:
                    self._durum(f"⬤  Tur {tur} — tıklanıyor…", self.YESIL)
                    butonlar = butonlari_bul(ayar["buton_renk"], ayar["renk_tolerans"])
                    if not butonlar:
                        self.log("⚠️ Ekranda buton bulunamadı!", "kirmizi")
                    else:
                        self.log(f"🎯 {len(butonlar)} buton bulundu.", "yesil")
            ...
            self._durum(f"⬤  Tur {tur} bitti — bekleniyor", self.SARI)
            bitis = time.time() + bekleme
            while not self.dur_event.is_set():
                kalan = int(bitis - time.time())
                if kalan <= 0: break
                self._geri_sayim_goster(f"⏳ Sonraki tur: {kalan} sn")
                time.sleep(1)
            self._geri_sayim_goster("")
    ...
    def _ekran_goruntus_al(self, tur, etiket, ayar):
        try:
            ...
            self.log(f"📸 Ekran görüntüsü hatası: {e}", "kirmizi")

    def _sonlandir(self):
        self.calisıyor = False
        self.root.after(0, lambda: self.btn_basla.config(
            text="▶  BAŞLAT", bg=self.BUTON_OF))
        self._durum("⬤  Durduruldu", self.ACCENT)
        self._geri_sayim_goster("")
